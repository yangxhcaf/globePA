...
p1 <- bio01_dat %>% ggplot() + 
  ...
  scale_y_continuous(expand=expansion(mult=c(0,.01)), limits=c(0,100), breaks=c(0, 1, 5, 15, 30, 50, 100)) +
...
p4 <- marspec_dat %>% filter(path=="biogeo13_bins") %>% left_join(area_val) %>%
  ...
  theme_bw() + labs(x="Mean annual SST (°C)", y="% of area protected") + 
  scale_x_continuous(expand=c(0,0)) + 
  scale_y_continuous(expand=expansion(mult=c(0,.01)), limits=c(0,100), breaks=c(0, 1, 5, 15, 30, 50, 100)) +
  scale_fill_manual(values=c("#D43F3AFF", "#EEA236FF", "#46B8DAFF", "#5CB85CFF"), 
                    guide = guide_legend(reverse = TRUE)) + 
...
p6 <- marspec_dat %>% filter(path == "bathy_bins") %>% left_join(area_val) %>%
  ...
  scale_x_continuous(expand=c(0,0)) + 
  scale_y_continuous(expand=expansion(mult=c(0,.01)), limits=c(0,100), breaks=c(0, 1, 5, 15, 30, 50, 100), 
                     sec.axis = sec_axis(~ .*362.2512, name="Area (10000 km2)", breaks=round(c(0, 1, 5, 15, 30)*362.2512, digits=0))) + 
  scale_fill_manual(values=c("#D43F3AFF", "#EEA236FF", "#46B8DAFF", "#5CB85CFF"), 
                    guide = guide_legend(reverse = TRUE)) + 
