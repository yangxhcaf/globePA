...
redblue <- c("grey", scico(7, palette = 'roma'))
...
ter_dat_ind <- readRDS("data/summary_ind_ter_perc.rds")
head(ter_dat_ind)
...
ter_dat_all <- readRDS("data/summary_all_ter_perc.rds")
...
files <- c("extdata/wc2.0_bio_30s_01_perc.nc", "extdata/wc2.0_bio_30s_12_perc.nc", "extdata/elevation_1KMmn_GMTEDmn_perc.nc")
dat <- raster::stack(files)
lapply(1:length(vars), function(x){
  if(!file.exists(paste0("extdata/", vars[x], "_protected_perc.nc"))){
    if(x == 1){
      dat_sub <- dat[[x]]
      dat_df <- ter_dat_ind %>% filter(path=="bio01_perc")
    } else if(x == 2){
      dat_sub <- dat[[x]]
      dat_df <- ter_dat_ind %>% filter(path=="bio12_perc")
    } else if(x == 3){
      dat_sub <- dat[[x]]
      dat_df <- ter_dat_ind %>% filter(path=="elevation_perc")
    } else if(x == 4){
      dat_df <- ter_dat_all %>% drop_na() %>% mutate(bio12 = bio12*1000) %>%
    ...
    raster::writeRaster(dat_sub, filename=paste0("extdata/", vars[x], "_protected_perc.nc"), compression=9, overwrite=T)
  }
})

# Load substituted raster files
prot_all <- raster::stack(paste0("extdata/", vars, "_protected_perc.nc"))
prot_all <- prot_all %>% fortify(maxpixels=2000000) %>% tidyr::drop_na()
...
prot_all <- prot_all %>% tidyr::drop_na() %>% gather(var, value, -c(x,y)) %>%
  mutate(perc2 = as.character(cut(value, breaks=c(0,1,2.5,7,16,25,40,100), include.lowest=T,
...
# Save figure
ggsave("figures/Figure4_perc.png", p1, width=8, height=6, dpi=1000)
ggsave("figures/Figure4.pdf", p1, width=8, height=6, dpi=1000)

ter_dat_ind %>% group_by(path) %>% summarise(sum(n))
ter_dat_ind %>% mutate(perc2 = as.character(cut(Total, breaks=c(0,1,2.5,7,16,25,40,100), include.lowest=T,
                                                labels=c("0 - 1", "1 - 2.5", "2.5 - 7", "7 - 16", "16 - 25",
                                                         "25 - 40", "40 - 100")))) %>% 
  mutate(perc2 = factor(if_else(Total == 0, "0", perc2),
                        levels=c("0", "0 - 1", "1 - 2.5", "2.5 - 7", "7 - 16", "16 - 25",
                                 "25 - 40", "40 - 100"))) %>% 
  group_by(path, perc2) %>% summarise(sum(n)/160809854*100) %>% view()

prot_all %>% filter(var=="(a) \t Temp") %>% filter(perc2 == "40 - 100") %>% 
...
prot_all %>% filter(var=="(a) \t Temp") %>% filter(perc2 == "2.5 - 7") %>% 
  ggplot() + geom_tile(aes(x=x, y=y, fill=perc2)) + 
  scale_fill_manual(values = redblue, name = "% Protected", na.value="transparent",
                    guide = guide_legend(reverse = TRUE)) + 
  geom_sf(data=outline_ter_moll, fill=NA, color = "black", size = 0.25/.pt) +
...
prot_all %>% filter(var=="(b) \t Prec") %>% filter(perc2 == "25 - 40") %>% 
  ggplot() + geom_tile(aes(x=x, y=y, fill=perc2)) + 
  scale_fill_manual(values = redblue, name = "% Protected", na.value="transparent",
                    guide = guide_legend(reverse = TRUE)) + 
  geom_sf(data=outline_ter_moll, fill=NA, color = "black", size = 0.25/.pt) +
...
prot_all %>% filter(var=="(c) \t Elevation") %>% 
  ggplot() + geom_tile(aes(x=x, y=y, fill=perc2)) + 
  scale_fill_manual(values = redblue, name = "% Protected", na.value="transparent",
                    guide = guide_legend(reverse = TRUE)) + 
  geom_sf(data=outline_ter_moll, fill=NA, color = "black", size = 0.25/.pt) +
...
ter_dat_all %>% group_by(bio01, bio12) %>% 
  ...
  mutate(perc2 = factor(if_else(perc == 0, "0", perc2),
                        levels=c("0", "0 - 1", "1 - 2.5", "2.5 - 7", "7 - 16", "16 - 25",
                                 "25 - 40", "40 - 100"))) %>%
  group_by(perc2) %>% summarise(sum(cells)/160809717*100)

prot_all %>% filter(var=="(d) \t Temp + Prec") %>% 
  filter(perc2 %in% c("0", "0 - 1", "1 - 2.5", "2.5 - 7")) %>% 
  ggplot() + geom_tile(aes(x=x, y=y, fill=perc2)) + 
  facet_wrap(var~., nrow=3) + 
  scale_fill_manual(values = redblue, name = "% Protected", na.value="transparent",
                    guide = guide_legend(reverse = TRUE)) + 
...
ter_dat_all %>% group_by(bio01, elevation) %>% mutate(prot_cells=(Total/100)*n) %>% 
  summarise(cells=sum(n), prot_cells=sum(prot_cells)) %>% 
  mutate(perc = prot_cells/cells*100) %>%
  mutate(perc2 = as.character(cut(perc, breaks=c(0,1,2.5,7,16,25,40,100), include.lowest=T,
                                  labels=c("0 - 1", "1 - 2.5", "2.5 - 7", "7 - 16", "16 - 25",
...
prot_all %>% filter(var=="(e) \t Temp + Elevation") %>% 
  filter(perc2 %in% c("25 - 40", "40 - 100")) %>% 
  ggplot() + geom_tile(aes(x=x, y=y, fill=perc2)) + 
  facet_wrap(var~., nrow=3) + 
  scale_fill_manual(values = redblue, name = "% Protected", na.value="transparent",
                    guide = guide_legend(reverse = TRUE)) + 
...
ter_dat_all %>% group_by(bio12, elevation) %>% mutate(prot_cells=(Total/100)*n) %>% 
  summarise(cells=sum(n), prot_cells=sum(prot_cells)) %>% 
  mutate(perc = prot_cells/cells*100) %>%
  mutate(perc2 = as.character(cut(perc, breaks=c(0,1,2.5,7,16,25,40,100), include.lowest=T,
                                  labels=c("0 - 1", "1 - 2.5", "2.5 - 7", "7 - 16", "16 - 25",
...
prot_all %>% filter(var=="(f) \t Prec + Elevation") %>% 
  filter(perc2 %in% c("0", "0 - 1", "1 - 2.5", "2.5 - 7")) %>% 
  ggplot() + geom_tile(aes(x=x, y=y, fill=perc2)) + 
  facet_wrap(var~., nrow=3) + 
  scale_fill_manual(values = redblue, name = "% Protected", na.value="transparent",
...
marspec_dat_ind <- readRDS("data/summary_ind_mar_perc.rds")
...
marspec_dat_all <- readRDS("data/summary_all_mar_perc.rds")
...
files <- c("extdata/biogeo13_30s_perc.nc", "extdata/biogeo08_30s_perc.nc", "extdata/bathy_30s_perc.nc")
dat <- stack(files)
lapply(1:length(vars), function(x){
  if(!file.exists(paste0("extdata/", vars[x], "_protected_perc.nc"))){
    if(x == 1){
      dat_sub <- dat[[x]]
      dat_df <- marspec_dat_ind %>% filter(path=="biogeo13_perc")
    } else if(x == 2){
      dat_sub <- dat[[x]]
      dat_df <- marspec_dat_ind %>% filter(path=="biogeo08_perc")
    } else if(x == 3){
      dat_sub <- dat[[x]]
      dat_df <- marspec_dat_ind %>% filter(path=="bathy_perc")
    } else if(x == 4){
...
prot_all <- prot_all %>% tidyr::drop_na() %>% gather(var, value, -c(x,y)) %>%
  ...
  mutate(perc2 = factor(if_else(value == 0, "0", perc2),
                        levels=c("0", "0 - 1", "1 - 2.5", "2.5 - 7", "7 - 16", "16 - 25",
                                 "25 - 40", "40 - 100")))
# Plot maps
p2 <- ggplot() + geom_tile(data=prot_all, aes(x=x, y=y, fill=perc2)) + 
  facet_wrap(var~., nrow=3) + 
  scale_fill_manual(values = redblue, name = "% Protected", na.value="transparent",
                    guide = guide_legend(reverse = TRUE)) + 
...
# Save figure
ggsave("figures/Figure5_perc.png", p2, width=9, height=6, dpi=1000)
ggsave("figures/Figure5.pdf", p2, width=9, height=6, dpi=1000)

marspec_dat_ind %>% group_by(path) %>% summarise(sum(n))
marspec_dat_ind %>% 
  mutate(perc2 = as.character(cut(Total, breaks=c(0,1,2.5,7,16,25,40,100), include.lowest=T,
                                  labels=c("0 - 1", "1 - 2.5", "2.5 - 7", "7 - 16", "16 - 25",
                                           "25 - 40", "40 - 100")))) %>%
  mutate(perc2 = factor(if_else(Total == 0, "0", perc2),
                        levels=c("0", "0 - 1", "1 - 2.5", "2.5 - 7", "7 - 16", "16 - 25",
                                 "25 - 40", "40 - 100"))) %>%
  group_by(path, perc2) %>% summarise(sum(n)/421197812*100) %>% as.data.frame() %>% head(30)

prot_all %>% filter(var=="(a) \t SST") %>% 
  ggplot() + geom_tile(aes(x=x, y=y, fill=perc2)) + 
  scale_fill_manual(values = redblue, name = "% Protected", na.value="transparent",
                    guide = guide_legend(reverse = TRUE)) + 
  geom_sf(data=outline_moll, fill=NA, color = "black", size = 0.25/.pt) + coord_sf() + theme_bw() + 
  theme(strip.background=element_blank(), axis.title=element_blank(), 
        strip.text = element_text(size=12, face="bold"), panel.border=element_blank(),
        axis.text = element_blank(), axis.ticks = element_blank())

prot_all %>% filter(var=="(b) \t SSS") %>% 
  ggplot() + geom_tile(aes(x=x, y=y, fill=perc2)) + facet_wrap(var~., nrow=3) + 
  scale_fill_manual(values = redblue, name = "% Protected", na.value="transparent",
                    guide = guide_legend(reverse = TRUE)) + 
  geom_sf(data=outline_moll, fill=NA, color = "black", size = 0.25/.pt) + coord_sf() + theme_bw() + 
  theme(strip.background=element_blank(), axis.title=element_blank(), 
...
prot_all %>% filter(var %in% c("(d) \t SST + SSS", "(e) \t SST + Bathy", "(f) \t SSS + Bathy")) %>% 
  filter(perc2 %in% c("16 - 25", "25 - 40", "40 - 100")) %>% 
  ggplot() + geom_tile(aes(x=x, y=y, fill=perc2)) + facet_wrap(var~., nrow=3) + 
  scale_fill_manual(values = redblue, name = "% Protected", na.value="transparent",
                    guide = guide_legend(reverse = TRUE)) + 
  geom_sf(data=outline_moll, fill=NA, color = "black", size = 0.25/.pt) + coord_sf() + theme_bw() + 
